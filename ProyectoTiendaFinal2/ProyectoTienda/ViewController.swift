...
class ViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        productoTotales.append(Productos(nombre: "Guayabas", presio: 134.0 , imagen: "guayaba",cantidad: 0))
        ...
        productoTotales.append(Productos(nombre: "Platano", presio: 35.0 , imagen: "platano",cantidad: 0))
        productoTotales.append(Productos(nombre: "Uva", presio: 10.0 , imagen: "uva",cantidad: 0))
        productoTotales.append(Productos(nombre: "Piñas", presio: 20.0 , imagen: "piña",cantidad: 0))
        productoTotales.append(Productos(nombre: "Peras", presio: 17.0 , imagen: "pera",cantidad: 0))
    }
    ...
    @IBAction func unwindToUno(_ segue: UIStoryboardSegue) {
        ...
        if let origin = segue.source as? ThirdViewController{
            carrito.removeAll()
            let data = origin.limpiar
            if (data == true){
                for i in 0...productoTotales.count - 1 {
                        productoTotales[i].cantidad = 0
                }
            }
